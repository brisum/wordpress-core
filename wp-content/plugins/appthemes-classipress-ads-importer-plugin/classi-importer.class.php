...
// Load Importer API
use Brisum\Lib\Excel\Xlsx\Document;
use Brisum\Lib\Excel\Xlsx\WorkBook;
...
class Classi_Importer extends WP_Importer {
	...
	function app_csv_to_array($file = '', $delimiter = ',') {
		...
		$tmpDir = sys_get_temp_dir() . '/' . mt_rand() . '-' . time();
		mkdir($tmpDir, 0777, true);

		$document = new Document($file, $tmpDir);
		$document->extract();

		$workBook = new WorkBook($document);
		$sheet = $workBook->getSheetById(1);
		$rows = $sheet->getRows(1, 100000);
		...
		foreach ($rows as $rowKey => $row) {
			...
			if (empty($ad['post_title'])) {
				die("Отсутствует заголов для обьявления. Строка #" . ($rowKey + 2). ".");
			}
			if (empty($ad['cp_author'])) {
				die("Отсутствует автор для обьявления. Строка #" . ($rowKey + 2) . ", название: {$ad['post_title']}.");
			}
			if (empty($ad['ad_cat'])) {
				die("Отсутствует категория для обьявления. Строка #" . ($rowKey + 2) . ", название: {$ad['post_title']}.");
			}
